  ...
  container.innerHTML = repos.map(r => {
    ...
    return `<div class="scroll-reveal glass-card rounded-2xl flex flex-col overflow-hidden shadow-lg">
      <div class="p-4 flex items-start gap-3 border-b border-[#263043]/50">
        <img src="${r.owner.avatar_url}" alt="" class="w-10 h-10 rounded-xl bg-slate-800 shrink-0" loading="lazy">
        <div class="min-w-0 flex-1">
          <div class="flex items-start justify-between gap-2">
            ...
            <span class="track-btn text-xs px-2 py-0.5 rounded-full border ${isTracked ? 'tracked' : 'border-slate-700 text-slate-500 hover:border-slate-500'}" data-action="track" data-repo="${r.full_name}">${isTracked ? '✓ Tracked' : '+ Track'}</span>
          </div>
        ...
        return `<tr class="scroll-reveal">
          ...
          <td>
            <div class="flex items-center gap-2.5 min-w-0">
              <img src="${r.owner.avatar_url}" alt="" class="w-7 h-7 rounded-lg bg-slate-800 shrink-0" loading="lazy">
              <div class="min-w-0">
